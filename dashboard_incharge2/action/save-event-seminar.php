...
if (isset($_POST['text'])) {
    ...
    $time = date("h:i A");
        ...
        $validate = "SELECT * FROM registered_users WHERE qrID = '$qrID' AND (Department = 'ENGINEERING' OR Department = 'none')";
    ...
    if ($count == 1) {
        ...
        $checkUser = "SELECT * FROM online_attendance2 WHERE email='$mail' AND Event_ID='$event_id'";
        $result = mysqli_query($conn, $checkUser);
        $count = mysqli_num_rows($result);
        
        if($count > 0){
            $_SESSION['validate'] = "existed";
            echo "<script>window.location.href='.?folder=pages/&page=event-scan2&userID=$event_id';</script>"; 
        } else {
            $scanned_data = array(
            ...
            if (check_internet_connection()) {
                ...
                $sql = "INSERT INTO online_attendance2 (Registered_ID, Event_ID, firstname, middlename, lastname, email, time_in, login_type, eventType) VALUES ('$reg_id', '$event_id', '$fname', '$mname', '$lname', '$mail', '$time','$type', 'SEMINAR')";
