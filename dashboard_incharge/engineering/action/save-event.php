...
if (in_array("EMPLOYEE STUDENT GUEST", $to)) {
    ...
    $query = mysqli_query($conn, "SELECT email FROM registered_users WHERE (login_type IN ('EMPLOYEE', 'STUDENT', 'GUEST') AND Department='ENGINEERING') OR (login_type = 'GUEST' AND Department = 'none')");
} elseif (in_array("STUDENT AND EMPLOYEE", $to)) {
    // If "STUDENT AND EMPLOYEE" is selected, retrieve both students and employees
    $query = mysqli_query($conn, "SELECT email FROM registered_users WHERE login_type IN ('STUDENT', 'EMPLOYEE') AND Department='ENGINEERING'");

}elseif (in_array("STUDENT AND GUEST", $to)) {
    // If "STUDENT AND GUEST" is selected, retrieve both students and guests
    $query = mysqli_query($conn, "SELECT email FROM registered_users WHERE login_type IN ('STUDENT', 'GUEST') AND Department='ENGINEERING' OR Department='none'");

}elseif (in_array("EMPLOYEE AND GUEST", $to)) {
    // If "EMPLOYEE AND GUEST" is selected, retrieve both employee and guests
    $query = mysqli_query($conn, "SELECT email FROM registered_users WHERE login_type IN ('EMPLOYEE', 'GUEST')AND Department='ENGINEERING' OR Department='none'");
    ...
    $query = mysqli_query($conn, "SELECT email FROM registered_users WHERE login_type IN ('STUDENT') AND Department='ENGINEERING' AND year='1st'");

}elseif (in_array("2ND YEAR", $to)) {
    // If "2ND YEAR" is selected, retrieve 2ND YEAR STUDENTS
    $query = mysqli_query($conn, "SELECT email FROM registered_users WHERE login_type IN ('STUDENT') AND Department='ENGINEERING' AND year='2nd'");

}elseif (in_array("3RD YEAR", $to)) {
    // If "3ND YEAR" is selected, retrieve 3RD YEAR STUDENTS
    $query = mysqli_query($conn, "SELECT email FROM registered_users WHERE login_type IN ('STUDENT') AND Department='ENGINEERING' AND year='3rd'");

}elseif (in_array("4TH YEAR", $to)) {
    // If "4TH YEAR" is selected, retrieve 4TH YEAR STUDENTS
    $query = mysqli_query($conn, "SELECT email FROM registered_users WHERE login_type IN ('STUDENT') AND Department='ENGINEERING' AND year='4th'");
}else {
    // Otherwise, retrieve the selected types only
    $query = mysqli_query($conn, "SELECT email FROM registered_users WHERE login_type IN ('" . implode("','", $to) . "') AND Department='ENGINEERING'");
}
...
foreach ($emails as $email) {
    ...
    $mail->Body    = "To: $towho\nFrom: $from\nSubject: $subject\nVenue: $venue\nDate: $date\nAgenda: $agenda";
    if ($file) {
        $mail->addAttachment('../assets/files/'.$file);
    }
    $mail->send();
    $mail->clearAddresses();
}
